...
def get_mask_location(model_type, category, model_parse: Image.Image, keypoint: dict, width=384,height=512):
    ...
    pose_width = keypoint.get("canvas_width", 384)
    pose_height = keypoint.get("canvas_height", 512)
    pose_scale = np.array([width / pose_width, height / pose_height], dtype=np.float32)
    ...
    if category == 'dresses' or category == 'upper_body':
        shoulder_right = pose_data[2][:2] * pose_scale
        shoulder_left = pose_data[5][:2] * pose_scale
        elbow_right = pose_data[3][:2] * pose_scale
        elbow_left = pose_data[6][:2] * pose_scale
        wrist_right = pose_data[4][:2] * pose_scale
        wrist_left = pose_data[7][:2] * pose_scale
