    ...
    def __call__(
        ...
        **kwargs,
        ...
        if ip_adapter_image is not None:
            ...
            # With CPU offload, this child module may still be on CPU even
            # though the prepared image embeddings are on the execution device.
            image_proj = self.unet.encoder_hid_proj
            image_proj_param = next(image_proj.parameters(), None)
            image_proj_original_device = (
                image_proj_param.device if image_proj_param is not None else torch.device(device)
            )
            image_proj_dtype = image_proj_param.dtype if image_proj_param is not None else image_embeds.dtype

            if image_proj_original_device != torch.device(device):
                image_proj.to(device)

            image_embeds = image_proj(image_embeds.to(device=device, dtype=image_proj_dtype)).to(
                device=device, dtype=prompt_embeds.dtype
            )

            if image_proj_original_device != torch.device(device):
                image_proj.to(image_proj_original_device)
